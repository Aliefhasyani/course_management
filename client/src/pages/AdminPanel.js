...
function AdminPanel({ user }) {
  ...
  const [currentCourse, setCurrentCourse] = useState(null); // Untuk edit kursus
  ...
  const [currentUser, setCurrentUser] = useState(null); // Untuk edit user
  const [userForm, setUserForm] = useState({
    username: '', email: '', role: '', password: '' // Password untuk opsional reset
  });
  ...
  const fetchData = async () => {
    ...
    setError('');
    try {
      const adminResponse = await getAdminPanelDataApi(user.role);
      setAdminData(adminResponse);

      const coursesResponse = await getCoursesApi();
      setCourses(coursesResponse);

      const usersResponse = await getAllUsersApi(user.role); // Memuat data user
  ...
  const handleCourseFormSubmit = async (e) => {
    ...
    try {
      if (currentCourse) {
        await updateCourseApi(currentCourse.id, courseForm, user.role);
        alert('Kursus berhasil diupdate!');
      } else {
        await createCourseApi(courseForm, user.role);
  ...
  const handleDeleteCourse = async (courseId) => {
    ...
    setError('');
    try {
      await deleteCourseApi(courseId, user.role);
  ...
  const validateUserForm = () => {
    ...
    // Password hanya wajib jika user baru atau jika ada perubahan
    if (currentUser && userForm.password && userForm.password.length < 6) errors.password = 'Password must be at least 6 characters.';
    else if (!currentUser && !userForm.password) errors.password = 'Password is required for new users.'; // Admin tidak membuat user baru di sini, tapi di Register page
  ...
  const handleUserFormSubmit = async (e) => {
    ...
    setError('');
    try {
      // Untuk user, kita hanya implement update. Register adalah untuk create.
      await updateUserApi(currentUser.id, userForm, user.role);
      alert('Pengguna berhasil diupdate!');
      setIsUserModalOpen(false);
      setCurrentUser(null);
      setUserForm({ username: '', email: '', role: '', password: '' });
      fetchData(); // Muat ulang data user
    } catch (err) {
  ...
  const openEditUserModal = (userToEdit) => {
    ...
    setUserForm({
      ...
      role: userToEdit.role || 'buyer', // Default ke 'buyer' jika tidak ada
      password: '' // Jangan tampilkan password lama, biarkan kosong untuk input baru
    });
    setUserFormErrors({});
  ...
  const handleDeleteUser = async (userId, usernameToDelete) => {
    ...
    setError('');
    try {
      await deleteUserApi(userId, user.role);
