...
@admin_required # Tambahkan decorator admin_required
def import_courses():
    ...
    for item in data:
        # Periksa apakah kursus dengan ID ini sudah ada
        existing_course = Course.query.get(item.get('id'))
        if not existing_course: # Hanya tambahkan jika belum ada
            course = Course(
                id=item.get('id'),
                sku=item.get('sku'),
                pic=item.get('pic'),
    ...
    db.session.commit()
    return jsonify({"message": "Courses imported successfully!"}), 201
...
# --- Rute CRUD Baru untuk Admin ---

@udemy_bp.route('/api/courses', methods=['POST'])
@admin_required # Hanya admin yang bisa membuat kursus baru
def create_course():
    ...
    new_course = Course(
        # ID akan otomatis di-generate jika tidak disertakan, atau kamu bisa memvalidasinya.
        # Untuk kasus ini, kita tidak meminta ID saat membuat, DB akan mengelola.
        sku=data.get('sku'),
        pic=data.get('pic'),
        ...
        rating=data.get('rating', 0.0), # Pastikan float
        duration=data.get('duration', 0.0), # Pastikan float
        expiry=data.get('expiry'),
        savedtime=data.get('savedtime') # Ini akan diupdate oleh DB secara default
    )
...
@admin_required # Hanya admin yang bisa mengupdate kursus
def update_course(course_id):
    ...
    # savedtime biasanya dihandle oleh DB atau tidak diupdate manual

    db.session.commit()
    return jsonify({"message": "Course updated successfully!"}), 200

@udemy_bp.route('/api/courses/<int:course_id>', methods=['DELETE'])
@admin_required # Hanya admin yang bisa menghapus kursus
